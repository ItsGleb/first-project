...
3. История коммитов ``` Bash $ git log ```, для получения сокращенного лога ```Bash $ git log --oneline``` . Самый последние коммиты будут первыми сверху
...
```mermaid
    ...
    staged --> modified: Внесение изменений
    modified --> staged: git add
    staged --> tracked: git commit
    tracked --> modified: Внесение изменений
    modified --> staged: git add
    staged --> tracked: git commit
    tracked --> modified: Внесение изменений
    modified --> [*]: Удаление файла```

> Этот цикл (`добавление → коммит → изменение`) составляет основу workflow в Git

### Какие состояния показывает `git status`

В типичном проекте большинство файлов находятся в состоянии **tracked** (отслеживаемые, закоммиченные и не изменённые после коммита). Однако:

- Состояние **tracked** не отображается в выводе `git status`  
  *(иначе команда выводила бы список всех файлов проекта)*

#### Состояния, которые **отображает** `git status`:

1. **staged**  
   `Changes to be committed`  
   Файлы, добавленные в staging area (`git add`), готовые к коммиту

2. **modified**  
   `Changes not staged for commit`  
   Изменённые отслеживаемые файлы, которые ещё не добавлены в staging area

3. **untracked**  
   `Untracked files`  
   Новые файлы, которые Git ещё не отслеживает

> 💡 Совет: Для краткого вывода используйте `git status -s` (флаг `--short`)